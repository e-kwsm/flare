...
from flare.dft_interface.cp2k_util import (
    parse_dft_input,
    parse_dft_forces,
    run_dft_par,
...
@pytest.mark.parametrize(
    "cp2k_input,exp_spec", [("./test_files/cp2k_input_1.in", ["H", "H"])]
)
...
@pytest.mark.parametrize(
    "cp2k_input,exp_cell", [("./test_files/cp2k_input_1.in", 5.0 * np.eye(3))]
)
...
@pytest.mark.parametrize(
    "cp2k_input,cp2k_output",
    [("./test_files/cp2k_input_1.in", "./test_files/cp2k_output_1.out")],
)
@pytest.mark.skipif(
...
def test_cp2k_calling(cp2k_input, cp2k_output):
    ...
    forces = run_dft_par("cp2k.in", structure, dft_loc)
