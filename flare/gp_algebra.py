...
def partition_force_energy_block(n_sample: int, size1: int, size2: int, n_cpus: int):
...
def parallel_matrix_construction(
    pack_function,
    hyps,
    name,
    kernel,
    ...
    for _ in range(nbatch):
        ...
        s1, e1, s2, e2 = block_id[wid]
        matrix[s1 * m1 : e1 * m1, s2 * m2 : e2 * m2] = result_chunk
...
def parallel_vector_construction(
    pack_function,
    name,
    x,
    kernel,
...
def multiple_array_construction(
    pack_function,
    name,
    x,
    kernel,
...
def get_force_energy_block_pack(
    hyps: np.ndarray,
    name: str,
    s1: int,
    e1: int,
...
def get_force_energy_block(
    ...
    if n_cpus is None:
        n_cpus = mp.cpu_count()
    if n_cpus == 1:
        force_energy_block = get_force_energy_block_pack(
            hyps, name, 0, size3, 0, size2, True, kernel, cutoffs, hyps_mask
        )
    else:
        # initialize matrices
        block_id, nbatch = partition_force_energy_block(n_sample, size3, size2, n_cpus)
        m1 = 3  # 3 force components per environment
        m2 = 1  # 1 energy per structure

        force_energy_block = parallel_matrix_construction(
            get_force_energy_block_pack,
            hyps,
            name,
            kernel,
            cutoffs,
...
def get_Ky_mat(
    ...
    force_block = get_force_block(
        hyps, name, force_kernel, cutoffs, hyps_mask, n_cpus, n_sample
    )

    energy_block = get_energy_block(
        hyps, name, energy_kernel, energy_noise, cutoffs, hyps_mask, n_cpus, n_sample
    )

    force_energy_block = get_force_energy_block(
        hyps, name, force_energy_kernel, cutoffs, hyps_mask, n_cpus, n_sample
    )
...
def update_force_block(
    ky_mat_old: np.ndarray,
    n_envs_prev: int,
    hyps: np.ndarray,
    name: str,
    kernel,
    ...
    if n_cpus == 1:
        ...
        new_mat = get_force_block_pack(
            hyps, name, 0, size, old_size, size, False, kernel, cutoffs, hyps_mask
        )
    ...
    # parallel version
        ...
        force_block = parallel_matrix_construction(
            get_force_block_pack,
            hyps,
            name,
            kernel,
    ...
    force_block[:old_size3, :old_size3] = ky_mat_old[:old_size3, :old_size3]
...
def update_energy_block(
    ...
    # parallel version
        ...
        energy_block = parallel_matrix_construction(
            get_energy_block_pack,
            hyps,
            name,
            kernel,
...
def update_force_energy_block(
    ...
    # serial version
    if n_cpus == 1:
        mat_1 = get_force_energy_block_pack(
            hyps,
            name,
            0,
            n_envs,
        ...
        mat_2 = get_force_energy_block_pack(
            hyps,
            name,
            n_envs_prev,
            n_envs,
    ...
    # parallel version
        ...
        block_id_1, nbatch_1 = partition_matrix_custom(
            n_sample, 0, n_envs, n_strucs_prev, n_strucs, n_cpus
        )
        block_id_2, nbatch_2 = partition_matrix_custom(
            n_sample, n_envs_prev, n_envs, 0, n_strucs_prev, n_cpus
        )
        ...
        mat_1 = parallel_matrix_construction(
            get_force_energy_block_pack,
            hyps,
            name,
            kernel,
        ...
        mat_2 = parallel_matrix_construction(
            get_force_energy_block_pack,
            hyps,
            name,
            kernel,
    ...
    # insert previous covariance matrix
    force_energy_block[: n_envs_prev * 3, :n_strucs_prev] = ky_mat_old[
        : n_envs_prev * 3, n_envs_prev * 3 : n_envs_prev * 3 + n_strucs_prev
    ]
...
def get_ky_mat_update(
    ...
    force_block = update_force_block(
        ky_mat_old,
        n_envs_prev,
        hyps,
        name,
    ...
    energy_block = update_energy_block(
        ky_mat_old,
        n_envs_prev,
        hyps,
        name,
    ...
    force_energy_block = update_force_energy_block(
        ky_mat_old,
        n_envs_prev,
        hyps,
        name,
...
def energy_energy_vector(
    ...
    if n_cpus is None:
        n_cpus = mp.cpu_count()
    if n_cpus == 1:
        return energy_energy_vector_unit(
    ...
    force_energy_vector = parallel_vector_construction(
        pack_function,
        name,
        x,
        kernel,
...
def energy_force_vector(
    ...
    if n_cpus is None:
        n_cpus = mp.cpu_count()
    if n_cpus == 1:
        return energy_force_vector_unit(
            name, 0, size, x, kernel, hyps, cutoffs, hyps_mask
        )
    ...
    k12_v = parallel_vector_construction(
        pack_function,
        name,
        x,
        kernel,
...
def efs_force_vector(
    ...
    efs_arrays = multiple_array_construction(
        pack_function,
        name,
        x,
        efs_force_kernel,
...
def get_kernel_vector(
    ...
    force_vector = force_force_vector(
        name, force_force_kernel, x, d_1, hyps, cutoffs, hyps_mask, n_cpus, n_sample
    )
    energy_vector = force_energy_vector(
        name, force_energy_kernel, x, d_1, hyps, cutoffs, hyps_mask, n_cpus, n_sample
    )
...
def get_ky_and_hyp(
    ...
    if n_cpus == 1:
        hyp_mat0, k_mat = get_ky_and_hyp_pack(
            name, 0, size, 0, size, True, hyps, kernel_grad, cutoffs, hyps_mask
        )
    else:
